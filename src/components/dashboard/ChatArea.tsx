...
export function ChatArea({ selectedContact, onRemoveFriend, sendWsMessage, webrtcRef, onStartCall }: ChatAreaProps) {
    ...
    <div className="flex-1 flex flex-col bg-white dark:bg-gray-900">
      ...
      <div className="h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Voice call */}
          <button
            onClick={() => onStartCall(selectedContact, false)}
            disabled={!webrtc.isConnected}
            className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            title={webrtc.isConnected ? "Chiamata vocale" : "Connetti P2P prima di chiamare"}
          >
          ...
          {/* Video call */}
          <button
            onClick={() => onStartCall(selectedContact, true)}
            disabled={!webrtc.isConnected}
            className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            title={webrtc.isConnected ? "Videochiamata" : "Connetti P2P prima di chiamare"}
          >
            <svg className="w-5 h-5 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
